...
export function generatePersonalizedPlans(answers) {
  ...
  // Calorie targets based on goal, weight, height, activity
  ...
  // Protein targets: higher for muscle gain or fitness goals
  ...
  // Custom meals selection based on dietary preference
  let meals = {
  ...
  // Workout exercises depending on Goal & Activity
  ...
  let habits = [
    ...
    { key: "sleep", title: `Sleep ${calorieTarget > 2000 ? 8 : 7.5} Hours`, target: `${calorieTarget > 2000 ? 8 : 7.5} hrs`, progress: `0 / ${calorieTarget > 2000 ? 8 : 7.5} hrs`, completed: false }
  ];
  ...
  return {
    ...
    completionPercent: 0, // start with 0% completion
...
export const planDb = {
  ...
  loadRecords: () => {
    ...
    if (!raw) {
      // Default placeholder records
      return { weightLogs: [], sleepLogs: [], waterLogs: [], moodLogs: [], workoutLogs: [] };
    }
    try {
      return JSON.parse(raw);
    } catch (e) {
