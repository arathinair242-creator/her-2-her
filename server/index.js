...
app.use((req, res, next) => {
  ...
  try {
    ...
    // Ignore logging file write errors in read-only filesystems
  }
  next();
});
...
// Database state middleware: Return clear 503 error if DB is disconnected instead of 10s buffering timeout
app.use('/api', (req, res, next) => {
  if (req.path === '/' || req.path === '') {
    return next();
  }
  if (mongoose.connection.readyState !== 1) {
    console.warn(`[DB WARN] Database not connected (readyState=${mongoose.connection.readyState}) for: ${req.method} ${req.url}`);
    return res.status(503).json({
      message: 'Database is connecting or unavailable. Please ensure MongoDB Atlas has Network Access 0.0.0.0/0 enabled and MONGODB_URI is set.'
    });
  }
  next();
});
...
app.get('/api', (req, res) => {
  ...
  res.json({ status: 'healthy', database: dbStatus, message: 'Her-2-Her Backend API is running...' });
});
